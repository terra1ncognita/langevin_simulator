...
public:
	...
	void advanceState(int nSteps, const double* rndNumbers) {
		...
		for (unsigned i = 0; i < nSteps; i++) {
			...
			double next_phi = _state.phi + (_sim.expTime / _mP.rotFriction) * (-_mP.rotStiffness*(_state.phi - _mP.iniPhi) + (_state.binding == 1.0) * (- molSpringForce * _mP.molLength*cos(_state.phi) + rot_pot_force)) + sqrt(2.0*_mP.kT*_sim.expTime / _mP.rotFriction) * rnd_phi;
